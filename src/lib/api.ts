...
/**
 * Родовые явления («Парламент», «Свадьба», «Депортация»…) — это понятия, а не события.
 * Сравниваем по префиксу, чтобы ловить любые словоформы: «парламента», «депортацию»…
 */
const GENERIC_PREFIXES = [
  "аннекси", "арест", "арми", "ассамб", "банк", "битв", "блокад", "бойкот",
  "войн", "выбор", "выставк", "газет", "город", "госуда", "дворц", "деклара",
  "депорта", "дефолт", "договор", "желез", "завод", "закон", "забасто",
  "замок", "импери", "инаугу", "инстит", "искусст", "истори", "казнь",
  "кафед", "кино", "коллед", "компани", "конститу", "концерт", "конфере",
  "корабл", "корона", "королев", "костел", "костёл", "кризис", "крушен",
  "культу", "мечет", "мечеть", "мост", "монасты", "музе", "обсерва",
  "объедине", "общест", "оккупа", "олимпиа", "опера", "орден", "остров",
  "открыти", "палат", "паломни", "памятни", "пандеми", "парламе", "партия",
  "перегово", "перемир", "перепись", "переворо", "пожар", "праздни",
  "правите", "предприя", "провозгла", "профсою", "раскол", "револю",
  "реформ", "республи", "саммит", "санкци", "собор", "соглаше", "сражен",
  "стадио", "статуя", "столиц", "суд", "судно", "свадьб", "театр",
  "универси", "фестива", "флот", "футбол", "храм", "церков", "школ",
  "эвакуа", "эмигра", "иммигра", "эмбарго", "экспеди", "эпидеми", "штаб",
];

/** Устойчивые родовые словосочетания (короткие, без уточнений — иначе зацепим «Гражданскую войну в США»). */
const GENERIC_PAIRS = [
  "гражданская война", "высший орган", "генеральный секретарь",
  "совет безопасности", "смертная казнь", "военное положение",
];
...
function isGenericTitle(norm: string): boolean {
  ...
  if (words.length <= 2) {
    const first = words[0];
    if (GENERIC_PREFIXES.some((p) => first.startsWith(p))) return true;
  }
  if (words.length <= 3) {
...
/**
 * «Понятие внутри описания»: если короткое (≤ 2 слов) название статьи целиком
 * входит в текст события, но вокруг него есть ещё ≥ 3 значимых слов, — это
 * термин из описания, а не статья о событии. «Депортация» во фразе
 * «…начата депортация французских поселенцев из Новой Шотландии…» — именно так.
 */
function isEmbeddedConcept(variant: string, leadNorm: string): boolean {
  const words = variant.split(" ").filter(Boolean);
  if (words.length === 0 || words.length > 2) return false;
  const idx = leadNorm.indexOf(variant);
  if (idx < 0) return false;
  const beforeOk = idx === 0 || leadNorm[idx - 1] === " ";
  const end = idx + variant.length;
  const afterOk = end >= leadNorm.length || leadNorm[end] === " ";
  if (!beforeOk || !afterOk) return false;
  const outside = (leadNorm.slice(0, idx) + " " + leadNorm.slice(end))
    .split(" ")
...
function scorePageForEvent(page: WikiPage, leadNorm: string): PageMatch | null {
  ...
  for (const v of variants) {
    if (v.length >= 8 && leadNorm.includes(v)) {
      // понятие, встроенное в описание более крупного события, — не статья о событии
      if (isEmbeddedConcept(v, leadNorm)) continue;
...
export async function findEventArticle(query: string): Promise<ArticleResult | null> {
  ...
  try {
    ...
    for (const r of results) {
      ...
      // …и о понятиях, встроенных в описание события («Депортация» в записи о депортации поселенцев)
      if (isEmbeddedConcept(norm, keyNorm)) continue;
